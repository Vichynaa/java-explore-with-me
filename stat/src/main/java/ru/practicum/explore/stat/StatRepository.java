...
public interface StatRepository extends JpaRepository<EndpointHit, Integer> {
    ...
    @Query(value = "SELECT * FROM hits WHERE uri = (SELECT uri FROM hits GROUP BY uri ORDER BY COUNT(uri) DESC LIMIT 1) AND timestamp BETWEEN :start AND :end", nativeQuery = true)
    List<EndpointHit> findTopUri(@Param("start") Timestamp start, @Param("end") Timestamp end);

    @Query(value = "SELECT h.* FROM hits h WHERE h.uri = (SELECT uri FROM hits GROUP BY uri ORDER BY COUNT(uri) DESC LIMIT 1) AND EXISTS (SELECT 1 FROM hits eh WHERE eh.uri = h.uri AND eh.timestamp BETWEEN :start AND :end GROUP BY eh.ip HAVING COUNT(DISTINCT eh.ip) > 0)", nativeQuery = true)
