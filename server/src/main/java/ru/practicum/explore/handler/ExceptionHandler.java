...
public class ExceptionHandler {
    ...
    public ResponseEntity<ErrorResponse> handleNumberFormatOverflow(final NumberFormatException e) {
        ErrorResponse errorResponse = new ErrorResponse(e.getMessage(), List.of(Arrays.toString(e.getStackTrace()).split("(?<=\\)), ")), "Incorrectly made request.", "BAD_REQUEST", LocalDateTime.now().format(Constant.getFORMATTER()));
        return new ResponseEntity<>(errorResponse, HttpStatus.BAD_REQUEST);
    }

    @org.springframework.web.bind.annotation.ExceptionHandler
    public ResponseEntity<ErrorResponse> handleValidationOverflow(final ValidationException e) {
        ErrorResponse errorResponse = new ErrorResponse(e.getMessage(), List.of(Arrays.toString(e.getStackTrace()).split("(?<=\\)), ")), "Main server error.", "BAD_REQUEST", LocalDateTime.now().format(Constant.getFORMATTER()));
        return new ResponseEntity<>(errorResponse, HttpStatus.BAD_REQUEST);
    }
    ...
    public ResponseEntity<ErrorResponse> handlerPSQLEOverflow(final org.postgresql.util.PSQLException e) {
        ...
        if (e.getMessage().contains("ERROR: duplicate key value violates unique constraint")) {
            errorResponse = new ErrorResponse(e.getMessage(), List.of(Arrays.toString(e.getStackTrace()).split("(?<=\\)), ")), "Main server error.", "CONFLICT", LocalDateTime.now().format(Constant.getFORMATTER()));
        }
